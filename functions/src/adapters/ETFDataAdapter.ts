...
import { ETFData, MarketType } from '../types/stock';
import type { YahooFinanceQuote, YahooFinanceHistoricalData } from 'yahoo-finance2';

// ETF 數據適配器
export class ETFDataAdapter extends BaseMarketDataAdapter {
  constructor() {
    super(MarketType.ETF);
  }
   ...
   * @returns ETF 數據
   */
  async fetchStockData(symbol: string): Promise<ETFData> {
    try {
   ...
   * @returns ETF 數據陣列
   */
  async fetchMultipleStocks(symbols: string[]): Promise<ETFData[]> {
