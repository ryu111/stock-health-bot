...
export class Validation {
  ...
  static isValidStockSymbol(symbol: string): boolean {
    ...
    return (
      twStockPattern.test(symbol) || usStockPattern.test(symbol) || hkStockPattern.test(symbol)
    );
  }
  ...
  static isValidBatchAnalysisRequest(request: unknown): boolean {
    ...
    for (const symbol of requestObj['symbols']) {
      if (!this.isValidStockSymbol(symbol as string) && !this.isValidETFSymbol(symbol as string)) {
        return false;
      }
    }
