...
class FlexMessageGenerator {
    ...
    generateStockAnalysisMessage(analysisResult, language = 'zh_TW') {
        const header = this.componentFactory.createHeader(analysisResult.symbol, analysisResult.marketType, language);
        const healthScoreCard = this.componentFactory.createHealthScoreCard(analysisResult.healthScore, language);
        const analysisCards = this.componentFactory.createAnalysisCards(analysisResult, language);
        const recommendationCard = this.componentFactory.createRecommendationCard(analysisResult.recommendation, language);
        const actionButtons = this.componentFactory.createActionButtons(analysisResult.symbol, language);
        return {
            type: 'box',
            layout: 'vertical',
            spacing: 'md',
    ...
    generateStockListMessage(stocks, language = 'zh_TW') {
        const header = this.componentFactory.createListHeader(language);
        const stockItems = stocks.map(stock => this.componentFactory.createStockListItem(stock, language));
    ...
    generateErrorMessage(error, language = 'zh_TW') {
    ...
    generateHelpMessage(language = 'zh_TW') {
        const helpItems = [
            {
    ...
    generateSettingsMessage(language = 'zh_TW') {
    ...
    generateWelcomeMessage(language = 'zh_TW') {
        return {
            type: 'box',
            layout: 'vertical',
            spacing: 'lg',
    ...
    generateBatchAnalysisMessage(analysisResults, language = 'zh_TW') {
        const header = {
            type: 'text',
        ...
        const stockCards = analysisResults.map(result => this.componentFactory.createCompactStockCard(result, language));
        return {
            type: 'box',
            layout: 'vertical',
            spacing: 'md',
            contents: [header, summary, ...stockCards],
        };
    }
    // 生成批量分析摘要
    generateBatchSummary(results, language) {
        const totalStocks = results.length;
        const avgHealthScore = Math.round(results.reduce((sum, r) => sum + r.healthScore, 0) / totalStocks);
    ...
    generateCompleteFlexMessage(contents, altText) {
        return {
            type: 'flex',
            altText: altText,
            contents: contents,
    ...
    // 生成多語系 Flex Message
    generateLocalizedFlexMessage(generator, _altTexts) {
